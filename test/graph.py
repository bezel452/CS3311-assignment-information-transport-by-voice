...
def displayWaveform(): # 显示语音时域波形
    ...
    :return:
    """
    samples, sr = librosa.load(r'D:/课程设计/noise.wav', sr=16000)
    ...
    plt.xlabel("时长（秒）")
    plt.ylabel("振幅")
    plt.savefig("D:/课程设计/123.png", dpi=600)
    plt.show()

def displaySpectrum(): # 显示语音频域谱线
    x, sr = librosa.load(r'D:/课程设计/test_re3.wav', sr=16000)
    print(len(x))
    # ft = librosa.stft(x)
    ...
    plt.savefig("D:/课程设计/whale5.png", dpi=600)
    # plt.show()
...
def displaySpectrogram():
    x, sr = librosa.load(r'D:/课程设计/noise.wav', sr=16000)
